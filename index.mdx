...
import { BetaBanner } from './components/beta-banner.tsx';
...
Used for partially updating objects. Where supported, Okta implements one or both of [JSON Patch](https://www.rfc-editor.org/rfc/rfc6902) and [JSON Merge Patch](https://www.rfc-editor.org/rfc/rfc7386)
...
The Okta API supports CORS on an API by API basis. If you're building an application that needs CORS, check that the specific operation supports CORS for your use case. APIs that support CORS are marked with an Icon.
